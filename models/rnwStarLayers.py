...
class UAdaDecoder(nn.Module):
    def __init__(self, num_ch_enc, style_dim):
        ...
        self.adaRes4 = AdainResBlk(512, 512, style_dim, w_hpf=1, upsample=True, mode="fourier_ap")
        # 3
        self.reduce3 = nn.Conv2d(self.num_ch_enc[3], 256, 1, bias=False)
        self.conv3 = Conv3x3(512, 256)
        self.adaRes3 = AdainResBlk(512, 256, style_dim, w_hpf=1, upsample=True, mode="nearest")
        # 2
        self.reduce2 = nn.Conv2d(self.num_ch_enc[2], 128, 1, bias=False)
        self.conv2 = Conv3x3(256, 128)
        self.adaRes2 = AdainResBlk(256, 128, style_dim, w_hpf=1, upsample=True, mode="nearest")
        # 1
        self.reduce1 = nn.Conv2d(self.num_ch_enc[1], 64, 1, bias=False)
        self.conv1 = Conv3x3(128, 64)
        self.adaRes1 = AdainResBlk(128, 64, style_dim, w_hpf=1, upsample=True, mode="nearest")
        # 0
        self.reduce0 = nn.Conv2d(self.num_ch_enc[0], 64, 1, bias=False)
        self.conv0 = Conv3x3(64, 64)
        self.adaRes0 = AdainResBlk(128, 64, style_dim, w_hpf=1, upsample=True, mode="fourier_ap")
