...
def parse_args():
    ...
    parser.add_argument('--checkpoint', type=str, default='checkpoints/rnw_star/checkpoint.ckpt')
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        for idx, item in enumerate(tqdm(image_list)):
            ...
            out_fn = os.path.join(visualization_dir, f'aug_{base}.png')
            ok=cv2.imwrite(out_fn, disps)
            if not ok:
                print("failed to write", out_fn)
